...
interface RatingColumnProps {
  ...
  onDrop: (techName: string, rating: UserRating["rating"]) => void;
  title: string;
}
...
export function RatingColumn({
  ...
  const [{ isOver }, drop] = useDrop({
    ...
    drop: (item: { name: string; type: string }) => onDrop(item.name, rating),
    collect: (monitor) => ({
  ...
  const filteredTechnologies = technologies.filter((tech) => {
    const userRating = userRatings.find((r) => r.technology === tech.name);
